...
if (contactForm) {
  contactForm.addEventListener('submit', (e) => {
    ...
    const courseSelect = document.getElementById('interestCourse');
    ...
    // Phone Validation (numeric, at least 10 digits)
    const phoneRegex = /^[0-9]{10,14}$/;
    const rawPhone = phoneInput.value.replace(/[\s\-()]/g, '');
    if (!rawPhone || !phoneRegex.test(rawPhone)) {
      document.getElementById('phoneError').innerText = 'Please enter a valid phone number (minimum 10 digits)';
      isValid = false;
    }

    // Course Selection
    if (!courseSelect.value) {
      document.getElementById('courseError').innerText = 'Please select a program of interest';
      isValid = false;
    }
    ...
    if (isValid) {
      ...
      // Scroll success box into viewport
      successBox.scrollIntoView({ behavior: 'smooth', block: 'nearest' });

      // Dismiss success box after 5 seconds
      setTimeout(() => {
